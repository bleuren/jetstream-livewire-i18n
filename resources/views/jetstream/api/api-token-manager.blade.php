<div>
    ...
    <x-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('jetstream.api.tokens.create') }}
        </x-slot>

        <x-slot name="description">
            {{ __('jetstream.api.tokens.description') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-label for="name" value="{{ __('jetstream.api.tokens.name') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-label for="permissions" value="{{ __('jetstream.api.tokens.permissions') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('jetstream.api.tokens.manage') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('jetstream.api.tokens.manage_description') }}
                </x-slot>
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        <x-slot name="title">
            {{ __('jetstream.api.tokens.token') }}
        </x-slot>

        <x-slot name="content">
            <div>
                {{ __('jetstream.api.tokens.token_copy_warning') }}
            </div>
    ...
    <x-confirmation-modal wire:model.live="confirmingApiTokenDeletion">
        <x-slot name="title">
            {{ __('jetstream.api.tokens.delete') }}
        </x-slot>

        <x-slot name="content">
            {{ __('jetstream.api.tokens.delete_warning') }}
        </x-slot>
