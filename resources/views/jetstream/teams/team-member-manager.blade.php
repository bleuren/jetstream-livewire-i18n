<div>
    @if (Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-form-section submit="addTeamMember">
                <x-slot name="title">
                    {{ __('jetstream.team.add_member') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('jetstream.role.description') }}
                </x-slot>

                <x-slot name="form">
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600 dark:text-gray-400">
                            {{ __('jetstream.role.provide_email') }}
                        </div>
                    </div>

                    <!-- Member Email -->
                    <div class="col-span-6 sm:col-span-4">
                        <x-label for="email" value="{{ __('jetstream.role.email') }}" />
                    ...
                    @if (count($this->roles) > 0)
                        <div class="col-span-6 lg:col-span-4">
                            <x-label for="role" value="{{ __('jetstream.role.title') }}" />
    ...
    @if ($team->teamInvitations->isNotEmpty() && Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('jetstream.invitation.pending') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('jetstream.invitation.description') }}
                </x-slot>
                                ...
                                <div class="flex items-center">
                                    @if (Gate::check('removeTeamMember', $team))
                                        ...
                                        <button class="cursor-pointer ms-6 text-sm text-red-500 focus:outline-none"
                                            ...
                                            {{ __('jetstream.invitation.cancel') }}
                                        </button>
                                    @endif
                                </div>
                            </div>
    ...
    @if ($team->users->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('jetstream.member.all') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('jetstream.member.description') }}
                </x-slot>
    ...
    <x-confirmation-modal wire:model.live="confirmingLeavingTeam">
        ...
        <x-slot name="content">
            {{ __('jetstream.team.leave_confirm') }}
        </x-slot>
